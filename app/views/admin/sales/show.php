...
<?php $verify = false; $subtotal = 0; $send = 0; $discount = 0; $user_id = $data['user_id'] ?>
    <h2 class="text-center">Detalles Carrito</h2>
    <form action="<?= ROOT ?>cart/update" method="POST">
        ...
        <table width="100%" class="text-end">
            <tr>
                ...
                <td width="9.20%" class="bg-danger text-white" ><?= number_format($subtotal, 2) ?></td>
            </tr>
            <tr>
                <td width="79.25%"></td>
                <td width="11.55%">Descuento:</td>
                <td width="9.20%" class="bg-dark text-white" ><?= number_format($discount, 2) ?></td>
            </tr>
            <tr>
                <td width="79.25%"></td>
                <td width="11.55%">Envío:</td>
                <td width="9.20%" class="bg-info text-white" ><?= number_format($send, 2) ?></td>
            </tr>
            <tr>
                <td width="79.25%"></td>
                <td width="11.55%"">Total:</td>
                <td width="9.20%" class="bg-primary text-white" ><?= number_format($total, 2) ?></td>
            </tr>
        </table>
    </form>
